...
public class Shop {
	...
	private void setPreparedStatements(Connection conn) {
		try {
			...
			selectTotalCost = conn.prepareStatement("SELECT P.pid AS pid, P.name AS name, P.price*S.quantity AS total FROM Products P, Sales S WHERE S.pid=P.pid AND S.cid= ?");
	...
	private Connection connect() {
		...
		// Connect To DataBase
		try {
			conn = DriverManager.getConnection("jdbc:oracle:thin:@dbs3.cs.umb.edu:1521:dbs3", "dpopovs", "WhiteJBgo");
	...
	private void menuP() {
		try {
			...
			while(queryResult.next()) {
				System.out.printf("%-5d | %-20s | %.2f\n", queryResult.getInt("pid"), queryResult.getString("name"), queryResult.getDouble("price"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	private void menuE() {
		try {
			...
			System.out.printf("%-5s | %-20s | %-10s\n", "ID", "NAME", "TOTAL");
			while(queryResult.next()) {
				System.out.printf("%-5d | %-20s | %-10.2f\n", queryResult.getInt("pid"), queryResult.getString("name"), queryResult.getDouble("total"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	private void menuB() {
		try {
			...
			queryResult = selectCustomerRow.executeQuery();
			queryResult.next();
			System.out.printf("Budget is: %.2f\n", queryResult.getDouble("budget"));
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
	...
	public static void main(String[] args) {
		...
		if(conn != null) {
			...
			}while(!shop.verifyCustomer());
			
			while(menuOption != 'x' && menuOption != 'X') {
				shop.displayMenu();
				menuOption = shop.input.next().charAt(0);
				System.out.println();
				if(menuOption == 'p' || menuOption == 'P') {
					shop.menuP();
				}else if (menuOption == 'o' || menuOption == 'O') {
					shop.menuO();
